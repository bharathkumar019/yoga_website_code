...
const linkAction = () =>{ 
    const navMenu = document.getElementById('nav-menu')
    // When we click on each nav__link, we remove the show-ment navMenu.classList.remove('show-menu')
    navMenu.classList.remove('show-menu')
}
...
const scrollUp = () =>{ 
  ...
  // When the scroll is higher than 350 viewport height, add the show-scre//
...
themeButton.addEventListener('click', () => {
  ...
  localStorage.setItem('selected-thene', getCurrentTheme())
...
sr.reveal(`.follow__data, .follow__content-1 img`,{origin: 'left'})